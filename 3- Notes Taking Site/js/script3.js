...
addNoteBtn.addEventListener("click", function () {
  ...
  if (currentNotePara.trim() != "" && currentNoteTitle.trim() != "") {
    ...
    let notesObj = {
        ...
        para:currentNotePara
    }
    notesArr.push(notesObj);

    addSingleElement(notesObj);
...
searchStr.addEventListener("input", function () {
  ...
  document.querySelectorAll(".note").forEach((el, i) => {
    ...
    if (notesArr[i].title.indexOf(searchStr) === -1) {
...
function addSingleElement(noteObj) {
  ...
  element.innerHTML = `<div class='my-3 card noteCard'>
    <div class='card-body'>
        ...
        <p class='card-text'>${noteObj.para}</p>
  ...
  notesArr.forEach((currentNote, i) => {
    ...
    element.innerHTML = `<div class='my-3 card noteCard'>
        <div class='card-body'>
            ...
            <p class='card-text'>${currentNote.para}</p>
